...
function createHighlightSquare(color, row, column){
    ...
    highlightsq.style = "background-color: rgb"+color+"; opacity: 0.55;";
...
function getTopUsername(){
    let topPlayerComponent = document.getElementsByClassName("player-component player-top").item(0);
    return topPlayerComponent.getElementsByClassName("user-tagline-username").item(0).innerHTML;
}
// get users most played openings from chess.com
function getUserGames(username){
    var xmlhttp = new XMLHttpRequest();
    let url = "https://api.chess.com/pub/player/"+username+"/games/2022/5/pgn";
    xmlhttp.onreadystatechange = function() {
        if(this.status == 200){
            var games = this.response;
            appendUserGames(games);
        }
...
function ECOtoName(eco){
    for(const name of ECO_CODES){
        if(name.eco == eco) return name.name;
    }
}
function appendUserGames(games){
    var openings = games.match(/\[ECO ".+"\]/g);
    if(openings == null) return;

    for(let i=0;i<openings.length;i++){
        openings[i] = openings[i].match(/[A-Z]\d{2}/)
    }
    var counts = {}
    for(const code of openings){
        counts[code] = counts[code] ? counts[code] + 1 : 1;
    }
    const top3 = Object
    .entries(counts) // create Array of Arrays with [key, value]
    ...
    var i=1;
    for(const openName of top3 ){
        appendElement.innerHTML += String(i++)+"."+ openName+"   ";
    }

    let topPlayerComponent = document.getElementsByClassName("player-component player-top").item(0);
    topPlayerComponent.append(appendElement);
...
function mainLoop(gameSettings){
   ...
   // check top username and do stuff if it changed.
   var topUserName = getTopUsername();
   if(topUserName != gameSettings["topUserName"]){
        gameSettings["topUserName"] = topUserName;
        let games = getUserGames(topUserName);
   }
...
function main(){
    ...
    setInterval(function () {mainLoop(gameSettings)}, 250);
}
